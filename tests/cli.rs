...
fn reference_check_validates_canonical_corpus_and_projections() {
    ...
    assert!(output.status.success());
    let stdout = String::from_utf8(output.stdout).unwrap();
    assert!(stdout.contains("entries=37\n"));
    assert!(stdout.contains("views=50\n"));
...
fn reference_sidecar_check_validates_relations_and_reviews() {
    ...
    assert!(stdout.contains("review_bindings=93\n"));
    assert!(stdout.contains("entry_review_bindings=37\n"));
    assert!(stdout.contains("view_review_bindings=50\n"));
