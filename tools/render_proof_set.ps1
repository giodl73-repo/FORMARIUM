...
if ($Edition -in @("sim-04", "sim-05", "sim-06", "sim-07")) {
    ...
    if ($numberedSelections.Count -ne 105 -or $guideSelections.Count -ne 2) {
...
<p>Search the 105 selected records and two application guides. Results open the canonical book projection below.</p>
...
'@
    ...
    $searchChecks = [ordered]@{
        ...
        duplicate_paths = 107 - $searchPaths.Count
...
if ($Edition -eq "sim-07") {
    ...
    if ($siteChapters.Count -ne 14 -or $chapterBySearchPath.Count -ne $searchRecords.Count) {
