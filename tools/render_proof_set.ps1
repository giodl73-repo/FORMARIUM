...
if ($Edition -in @("sim-04", "sim-05", "sim-06", "sim-07")) {
    ...
    if ($numberedSelections.Count -ne 99 -or $guideSelections.Count -ne 2) {
...
<p>Search the 99 selected records and two application guides. Results open the canonical book projection below.</p>
<div class="proof-search__controls">
<label for="proof-search-query">Search terms
<input id="proof-search-query" type="search" autocomplete="off" placeholder="Try force, sample, causality, workflow, or contract">
</label>
...
'@
    ...
    $searchChecks = [ordered]@{
        ...
        duplicate_paths = 101 - $searchPaths.Count
...
if ($Edition -eq "sim-07") {
    ...
    if ($siteChapters.Count -ne 11 -or $chapterBySearchPath.Count -ne $searchRecords.Count) {
