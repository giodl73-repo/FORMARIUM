...
if ($Edition -in @("sim-04", "sim-05", "sim-06", "sim-07")) {
    ...
    if ($numberedSelections.Count -ne 116 -or $guideSelections.Count -ne 2) {
...
<p>Search the 116 selected records and two application guides. Results open the canonical book projection below.</p>
...
'@
    ...
    $searchChecks = [ordered]@{
        ...
        duplicate_paths = 118 - $searchPaths.Count
...
if ($Edition -eq "sim-07") {
    ...
    if ($siteChapters.Count -ne 19 -or $chapterBySearchPath.Count -ne $searchRecords.Count) {
