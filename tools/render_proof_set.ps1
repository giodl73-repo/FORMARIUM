...
if ($Edition -in @("sim-04", "sim-05", "sim-06", "sim-07")) {
    ...
    if ($numberedSelections.Count -ne 120 -or $guideSelections.Count -ne 2) {
...
<p>Search the 120 selected records and two application guides. Results open the canonical book projection below.</p>
...
'@
    ...
    $searchChecks = [ordered]@{
        ...
        duplicate_paths = 122 - $searchPaths.Count
...
if ($Edition -eq "sim-07") {
    ...
    if ($siteChapters.Count -ne 23 -or $chapterBySearchPath.Count -ne $searchRecords.Count) {
