...
const current = JSON.parse(read("fixtures/lexical-closure/lookup-alias-result-01.json"));
...
assert.match(context, /internal Proof Set lane is now maintained and stopped at clean `sim-47`/);
assert.match(roadmap, /maintain clean `sim-47` as the two-book internal reference/);
assert.match(audit, /Maintained edition: `sim-47`/);
...
assert.match(proofSet, /Status: internal rehearsal package maintained at `sim-47`; external preview deferred/);
...
console.log("OK campaign=PS-FP-46 compatibility=retained authorities=4 maintained=sim-47 history=sim-46 preview-tag=absent claims=internal-only");
