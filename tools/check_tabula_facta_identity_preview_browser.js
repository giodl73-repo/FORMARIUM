...
const siteRoot = path.resolve(process.argv[2] || "target/proof-set-sim-51");
const screenshot = path.resolve(process.argv[3] || "target/sim51-tabula-facta-reader.png");
assert.equal(JSON.parse(fs.readFileSync(path.join(siteRoot, "manifest.json"), "utf8")).edition, "sim-51");
...
const port = 10440 + (process.pid % 40);
const profile = path.resolve(`target/edge-tabula-facta-${process.pid}`);
...
async function evaluate(client, expression) { const response = await client.call("Runtime.evaluate", {expression, returnByValue: true}); if (response.exceptionDetails) throw new Error(response.exceptionDetails.text); return response.result.value; }
...
(async () => {
  ...
  try {
    ...
    for (let attempt = 0; attempt < 100 && !(await evaluate(client, "document.readyState === 'complete'")); attempt += 1) await delay(60);
    const state = await evaluate(client, `(() => ({brand:document.querySelector('.site-brand').textContent.trim(),banner:document.querySelector('.identity-preview').textContent.trim(),heading:document.querySelector('h1').textContent.trim(),authority:document.querySelector('.reader-route__boundary').textContent.trim(),start:document.querySelector('[data-reader-start="sequence"]').getAttribute('href')}))()`);
    assert.match(state.brand, /Tabula Facta.*candidate/);
    assert.match(state.banner, /not a locked rename/i);
    assert.equal(state.heading, "The Factorium Reader");
    assert.match(state.authority, /Factor Tables remain authoritative/);
    ...
    console.log(`OK reader-brand=Tabula-Facta heading=Factorium-Reader mobile=390 screenshot=${screenshot}`);
