...
assert.equal(manifest.composition_lab_checks.canonical_relation_records, 11,
  "canonical graph reports its five separately reviewed cross-entry relations");
...
assert.ok(!payload.relations.some((relation) =>
  relation.id === "f27-value-contributes-criterion"),
"value canonical edge remains outside interactive Lab");
assert.ok(!payload.relations.some((relation) =>
  relation.id === "f27-risk-characterizes-consequence"),
"risk canonical edge remains outside interactive Lab");
assert.ok(!payload.relations.some((relation) =>
  relation.id === "f27-causal-scope-qualifies-outcome"),
"causal canonical edge remains outside interactive Lab");
assert.ok(!payload.relations.some((relation) =>
